	...
	return this.each( function() {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				cn1 = (altr+j)%2 == 1 ? cn : '';
				rowData.push( "<tr"+hiderow+" id=\""+rid+"\" tabindex=\"-1\" role=\"row\" class =\"ui-widget-content-grid jqgrow ui-row-"+ts.p.direction+""+cn1+"\">" );
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				altr = rcnt === 1 ? 0 : rcnt;
				cn1 = (altr+i)%2 == 1 ? cn : '';
				rowData.push("<tr"+hiderow+" id=\""+ idr +"\" tabindex=\"-1\" role=\"row\" class= \"ui-widget-content-grid jqgrow ui-row-"+ts.p.direction+""+cn1+"\">");
				if(ts.p.rownumbers===true) {
        ...
        setPager = function (pgid, tp){
			...
			$(document.body).append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content-grid' style='font-size:"+tdw+";visibility:hidden;' ></div>");
		...
		var	tfoot = "";
		if(ts.p.footerrow) { tfoot += "<table role='grid' style='width:"+ts.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content-grid footrow footrow-"+dir+"'>"; }
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					row = "<tr id=\""+rowid+"\" role=\"row\" tabindex=\"-1\" class=\"ui-widget-content-grid jqgrow ui-row-"+t.p.direction+" "+cna+"\">" + row+"</tr>";
					if(t.rows.length === 0){
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {		
		...
		$(mc).addClass("ui-jqdialog-content ui-widget-content-grid").attr("id",aIDs.modalcontent);
		$(mc).append(content);
		mw.appendChild(mc);
		$(mw).prepend(mh);
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		cnt += c_b ? "<div class='ui-widget-content-grid ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a href='javascript:void(0)' id='closedialog' class='fm-button ui-state-default ui-corner-all'>"+c_b+"</a>"+buttstr+"</div>" :
			buttstr !== ""  ? "<div class='ui-widget-content-grid ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>"+buttstr+"</div>" : "";
		cnt += "</div>";
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				if(typeof(options.dataUrl) != "undefined") {
					if(options.dataUrl.indexOf('?') == -1){
						options.dataUrl = options.dataUrl + "?cellValue="+document.getElementById('cellValue').value;
					}
					else{
						options.dataUrl = options.dataUrl + "&cellValue=中国";
					}
					//处理get方式乱码,后台使用 URLDecoder.decode("你的字符串","UTF-8"); 
					options.dataUrl = encodeURI(options.dataUrl); 
					options.dataUrl = encodeURI(options.dataUrl); 
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		if(this.p.showQuery) {
			$(this).append("<table class='queryresult ui-widget ui-widget-content-grid' style='display:block;max-width:440px;border:0px none;'><tbody><tr><td class='query'></td></tr></tbody></table>");
		}
		/*
		 *Perform checking.
		 *
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				bt = "<table class='EditTable' style='border:0px none;margin-top:5px' id='"+fid+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content-grid' style='margin:1px'/></td></tr><tr><td class='EditButton' style='text-align:left'>"+bC+tmpl+"</td><td class='EditButton'>"+bQ+bS+"</td></tr></tbody></table>";
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(this.edittype == 'text' || this.edittype == 'textarea') {
							$(elc).addClass("ui-widget-content-grid ui-corner-all");
						}
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				var bt = "<table border='0' cellspacing='0' cellpadding='0' class='EditTable' id='"+frmtb+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content-grid' style='margin:1px'/></td></tr><tr id='Act_Buttons'><td class='navButton'>"+(rtlb ? bN+bP : bP+bN)+"</td><td class='EditButton'>"+bS+bC+"</td></tr>";
				...
				if(rp_ge.checkOnSubmit || rp_ge.checkOnUpdate) {
					...
					$("<div class='ui-widget-overlay jqgrid-overlay confirm' style='z-index:"+zI+";display:none;'>&#160;"+ii+"</div><div class='confirm ui-widget-content-grid ui-jqconfirm' style='z-index:"+(zI+1)+"'>"+p.saveData+"<br/><br/>"+bS+bN+bC+"</div>").insertAfter("#"+frmgr);
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				tdtmpl = "<td class='CaptionTD form-view-label ui-widget-content-grid' width='"+p.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content-grid'>&#160;</td>", tmpl="",
				tdtmpl2 = "<td class='CaptionTD form-view-label ui-widget-content-grid'>&#160;</td><td class='DataTD form-view-data ui-widget-content-grid'>&#160;</td>",
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				tbl += "<table cellspacing='0' cellpadding='0' border='0' class='EditTable' id='"+dtbl+"_2'><tbody><tr><td><hr class='ui-widget-content-grid' style='margin:1px'/></td></tr></tr><tr><td class='DelButton EditButton'>"+bS+"&#160;"+bC+"</td></tr></tbody></table>";
				p.gbox = "#gbox_"+gID;
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				tdw = $(".ui-jqgrid").css("font-size") || "11px";
				$('body').append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content-grid' style='font-size:"+tdw+";visibility:hidden;' ></div>");
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							...
							if(bfsc === false) {return false;}
								$(tr).after( "<tr role='row' class='ui-subgrid'>"+atd+"<td class='ui-widget-content-grid subgrid-cell'><span class='ui-icon "+ts.p.subGridOptions.openicon+"'></span></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='ui-widget-content-grid subgrid-data'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.sortitems[0],function(i,n){
				...
				str += "<tr id=\""+hid+"\" role=\"row\" class= \"ui-widget-content-grid jqgroup ui-row-"+$t.p.direction+"\"><td colspan=\""+colspans+"\">"+icon+$.jgrid.format(grp.groupText[0], gv, grdata[n].length)+"</td></tr>";
				...
				if(grp.groupSummary[0]) {
					...
					str += "<tr"+hhdr+" role=\"row\" class=\"ui-widget-content-grid jqfoot ui-row-"+$t.p.direction+"\">";
