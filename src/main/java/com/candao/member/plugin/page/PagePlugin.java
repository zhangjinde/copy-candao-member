...
public class PagePlugin implements Interceptor {
	...
	public Object intercept(Invocation ivk) throws Throwable {
		...
		if(ivk.getTarget() instanceof RoutingStatementHandler){
			...
			if(true == context.isPagination()) { //拦截需要分页的SQL
	...
	private String generatePageSql(String sql,Page page){
		if(page!=null && Tools.notEmpty(dialect)){
			...
			if("mysql".equals(dialect)){
				pageSql.append(sql);
				pageSql.append(" limit "+page.getPageStartRow()+","+page.getPageSize());
			}else if("oracle".equals(dialect)){
				pageSql.append("select * from (select tmp_tb.*,ROWNUM row_id from (");
				pageSql.append(sql);
				//pageSql.append(") as tmp_tb where ROWNUM<=");
				pageSql.append(") tmp_tb where ROWNUM<=");
				pageSql.append(page.getPageStartRow()+page.getPageSize());
				pageSql.append(") where row_id>");
				pageSql.append(page.getPageStartRow());
			}
