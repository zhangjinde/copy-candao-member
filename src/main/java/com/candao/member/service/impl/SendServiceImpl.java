...
import com.candao.member.log.LoggerFactory;
import com.candao.member.log.LoggerHelper;
import com.candao.member.mail.SimpleMailSender;
import com.candao.member.service.SendService;
import com.candao.member.utils.AccountUtils;
import com.candao.member.utils.BusinessException;
...
//	}
  ...
  //change by 2016-07-08 添加返回短信状态
  @Override
  public String sendAccountBySms(HttpServletRequest request,HttpServletResponse response,String telephone, String account) throws IOException {
	//每次发送账户都需要随机设置用户密码，设置密码失败时
    String password = AccountUtils.getRandomPassword();
//	if (userDao.updatePassword(account, MD5.md5(password)) > 0) {
...
//			//放入session
			request.getSession().setAttribute("valicode", password);
...
//		}
		...
		return sendValicodeBySms(telephone,password);
 ...
 * @throws AddressException 
  ...
  /**
   *  通过短信发送验证码
   * @param paramMap 参数 change by 2016-07-08 添加返回短信状态
 * @throws IOException  
   */
  public String sendValicodeBySms(String mobile, String valicode) throws IOException {
	  Map<String,String> param = this.getSmsParam();
	  if(param!=null){
